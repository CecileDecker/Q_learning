...
def robust_q_learning_v2(X, A, r, P_0, p_0, alpha, x_0, k_0, eps_greedy = 0.05, Nr_iter = 1000, gamma_t_tilde = lambda t: 1/(t+1), Q_0 = None):
    """
    Parameters
    ----------
    X : numpy.ndarray
    ...
    for t in tqdm(range(Nr_iter)):
        ...
        for k in range(len(Y_0)):
            Y_1 += [P_(Y_0[k], A_0[k], k)] # Update the Markov decision processes
